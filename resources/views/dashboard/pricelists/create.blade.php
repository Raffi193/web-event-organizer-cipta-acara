<x-layouts.app :title="__('Form Tambah Pricelist')">
  <div class="flex flex-col h-full w-full flex-1 gap-4 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
    ...
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('Form Tambah Pricelist') }}</h2>
    </div>
    ...
    <form action="{{ route('pricelists.store') }}" method="POST">
      ...
      <div class="mb-6">
        ...
        <input type="text" id="title" name="title"
          ...
          value="{{ old('title') }}" placeholder="Isi nama pricelist" required>
        @error('title')
      ...
      <div class="mb-6">
        ...
        <input type="number" id="price" name="price"
          class="bg-gray-50 dark:bg0 border border-gray-300 text-gray-900 text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 dark:bg-neutral-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
          value="{{ old('price') }}" placeholder="Isi harga pricelist" required>
        @error('price')
          <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
      <div class="mb-6">
        ...
        <textarea rows="4" id="description" name="description"
          class="bg-gray-50 dark:bg0 border border-gray-300 text-gray-900 text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 dark:bg-neutral-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
          value="{{ old('description') }}" placeholder="Isi deskripsi pricelist" required>{{ old('description') }}</textarea>
        @error('description')
          <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
      ...
      <div class="mb-6">
        ...
        <select name="photographer_slug" id="photographer_slug"
          ...
          @foreach ($photographers as $photographer)
            <option value="{{ $photographer->slug }}" {{ old('photographer_slug') == $photographer->slug ? 'selected' : '' }}>
              {{ $photographer->name }}
            </option>
          @endforeach
        </select>
      ...
      <div class="mb-6">
        ...
        <select name="mc_slug" id="mc_slug"
          class="bg-gray-50 dark:bg-neutral-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 dark:placeholder-gray-400">
          @foreach ($mcs as $mc)
            <option value="{{ $mc->slug }}" {{ old('mc_slug') == $mc->slug ? 'selected' : '' }}>
              {{ $mc->name }}
            </option>
          @endforeach
        </select>
      ...
      <div class="mb-6">
        ...
        <select name="decoration_slug" id="decoration_slug"
          ...
          @foreach ($decorations as $decoration)
            <option value="{{ $decoration->slug }}" {{ old('decoration_slug') == $decoration->slug ? 'selected' : '' }}>
              {{ $decoration->name }}
            </option>
          @endforeach
        </select>
