<x-layouts.app :title="__('Form Ubah ' . $title)">
  <div class="flex flex-col h-full w-full flex-1 gap-4 rounded-xl border border-neutral-200 dark:border-neutral-700 p-6">
    ...
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-xl font-semibold text-gray-900 dark:text-white">{{ __('Form Ubah ' . $title) }}</h2>
    </div>

    <form action="{{ route($route . '.update', $vendor->slug) }}" method="POST">
      @csrf
      @method('put')
      <div class="mb-6">
        ...
        <input type="text" id="name" name="name"
          ...
          value="{{ old('name', $vendor->name) }}" placeholder="Isi nama {{ $title }}" required>
        @error('name')
      ...
      <div class="mb-6">
        ...
        <input type="text" id="services" name="services"
          class="bg-gray-50 dark:bg0 border border-gray-300 text-gray-900 text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 dark:bg-neutral-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
          value="{{ old('services', $vendor->services) }}" placeholder="Isi layanan {{ $title }}" required>
        @error('services')
          <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
      <div class="mb-6">
        ...
        <input type="text" id="contact" name="contact"
          class="bg-gray-50 dark:bg0 border border-gray-300 text-gray-900 text-sm rounded focus:ring-indigo-500 focus:border-indigo-500 block w-full p-3 dark:bg-neutral-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white"
          value="{{ old('contact', $vendor->contact) }}" placeholder="Isi kontak {{ $title }}" required>
        @error('contact')
          <p class="mt-2 text-xs text-red-600">{{ $message }}</p>
        @enderror
      </div>
